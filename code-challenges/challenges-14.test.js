...
const toTitleCase = (arr) => {
  // Solution code here...
  let result=[];
  let mappingArr=arr.map(element=>{
    let first=element.charAt(0).toUpperCase();
    let b=first+element.substring(1);
    result.push(b);
  });
  return mappingArr,result;
};
...
let biggerThanLuke = (arr) => { //compare  more than 75
  let res=[];
  arr.filter(element =>{
      let a=arr[0].mass;
      console.log(a);
    if(Number(element.mass) > Number(a)){ //compare 
    console.log(element+ '   '+ element.mass)
      return res.push(element.name);
    }
    else{
      return 0;
    }
