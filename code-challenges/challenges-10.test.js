...
Build a simple express server. Connect a '/hello' route that sends a greeting of your  choice. Connect a '/aboutme' route that sends a short bio about you to the front-end. 
...
const createServer = () => {
  ...
  let app = express();
  app.get('/hello', function greeting(req, res) {
    res.send('hello from backend ....').catch(err=>{
    res.stats(500).send('sorry some error occured ',err);
  });});

  app.get('/aboutme', function greeting(req, res) {
    res.send('hello  i am hadeel ').catch(err=>{
    res.stats(500).send('sorry some error occured ',err);
  });});

  app.get('/favoritefoods', function favFood(req, res) {
    let food=['mansaf','kebab','spicyfood'];
    let foodArr=food.map(ele=>{
      return ele;
    });
    res.send(foodArr).catch(err=>{
    res.stats(500).send('sorry some error occured ',err);
  });});
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let fill=input.map(ele=>{
    for(let i=0;i<input.length;i++){
       let anotherArray=ele.reduce((acc,ele2)=>{
        if(ele2%5===0 && typeof(ele2)==='number'){
          let pow2=Math.pow(2,ele2);
         acc.push(pow2);
        }
        return acc;
        },[]);
        ...
        console.log('another    '+anotherArray +'     ' + typeof(anotherArray));
